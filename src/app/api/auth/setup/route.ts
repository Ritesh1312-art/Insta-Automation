...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!receivedToken || !expectedToken || receivedToken.length !== expectedToken.length || !timingSafeEqual(receivedToken, expectedToken)) return NextResponse.json({ error: 'Invalid setup token' }, { status: 401 });
